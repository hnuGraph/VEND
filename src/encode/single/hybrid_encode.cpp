...
PairType HybridEncode::NonNTestSIMD(uint32_t vertex1, uint32_t vertex2) {}
...
void HybridEncode::Decode(uint32_t vertex, DecodeInfo &decode_info) {
    ...
    if (encode->IsOne(0)) {
        ...
        if (decode_info.block_num == 0) {
            ...
            if (type & 0x0b)
                decode_info.max =
                    encode->BlockGet(BLOCK_BEGIN_INDEX + (decode_info.block_num - 1) * v_bits_size_, v_bits_size_);
            else
...
void HybridEncode::EncodeVertex(uint32_t vertex_id, std::vector<uint32_t> &neighbors) {
    ...
    if (neighbors.size() <= MAX_K_CORE_SIZE) {
        ...
        DynamicChoose(vertex_id, neighbors, score);
    }
}
...
void HybridEncode::InsertPair(uint32_t vertex1, uint32_t vertex2) {
    ...
    std::vector<uint32_t> neighbor2 = GetAllNeighbors(vertex2), neighbor1 = GetAllNeighbors(vertex1);
...
void HybridEncode::RestructChoose(uint32_t vertex1, std::vector<uint32_t> &neighbor1, uint32_t vertex2,
                                  std::vector<uint32_t> &neighbor2) {
    uint32_t score1, score2;
    //    BitSetSIMD *bitset1 = new BitSetSIMD(PER_ENCODE_BIT_SIZE);
    //    BitSetSIMD *bitset2 = new BitSetSIMD(PER_ENCODE_BIT_SIZE);
    DynamicChoose(vertex1, neighbor1, score1);
    DynamicChoose(vertex2, neighbor2, score2);
    //    if (score1 > score2)
    //        encode_bitset_[vertex1]->Reconstruct(bitset1);
    //    else
    //        encode_bitset_[vertex2]->Reconstruct(bitset2);
    //    delete bitset1;
    //    delete bitset2;
}
...
void HybridEncode::DynamicChoose(uint32_t vertex, const std::vector<uint32_t> &neighbors, uint32_t &count) {
    uint32_t max_score = 0, neighbor_size = neighbors.size();

    BitSetSIMD *bitset = encode_bitset_[vertex];
...
void HybridEncode::ChooseHighest(const std::vector<uint32_t> &neighbors, uint32_t &max_score, BitSetSIMD &best_bitset,
                                 HashCount *&hash_count, uint32_t block_size, uint32_t first_idx,
    ...
    if (max_score < score) {
        ...
        std::vector<uint32_t> neighbor_left;
        if (first_idx > 0) neighbor_left.insert(neighbor_left.end(), neighbors.begin(), neighbors.begin() + first_idx);
...
void HybridEncode::SetNonDecodable(const std::vector<uint32_t> &neighbor_encode,
                                   const std::vector<uint32_t> &neighbor_left, const BlockType &type,
                                   BitSetSIMD &bitset) {
    bitset.Clear();
    bitset.SetZero(0);
    ...
    if (hash_begin == PER_ENCODE_BIT_SIZE) return;
...
std::vector<uint32_t> HybridEncode::GetBlockInteger(uint32_t vertex) {
    ...
    if (IsDecodable(vertex)) {
        for (uint32_t index = 1; index <= PER_ENCODE_BIT_SIZE - v_bits_size_ + 1; index += v_bits_size_) {
            ...
            if (!neighbor) return block_integers;
            block_integers.push_back(neighbor);
        }

    } else {
        for (uint32_t index = 3 + log_k_; index < 3 + log_k_ + GetBlockSize(vertex) * v_bits_size_;
             index += v_bits_size_) {
...
// TODO
uint32_t HybridEncode::NePairCount(uint32_t vertex_id, const BitSetSIMD &encode) { return 0; }
