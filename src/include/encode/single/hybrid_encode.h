 ...
 *  encode format (bit)
 *  ------------------------------------------------------------------------------------------------------------------
 *  | FULL FLAG (1) | RANGE MODEL (2) | RANGE SIZE (log(k))|  RANGE ENCODE PART (RANGE SIZE * VERTEX BIT)| HASH ENCODE
 * PART
 *  -------------------------------------------------------------------------------------------------------------------
 *
 *
...
class HybridEncode : public EncodeBitSet {
   public:
    HybridEncode()
        : log_k_(LOG_K), v_bits_size_(VERTEX_BIT_SIZE), max_integer_size_(MAX_INTEGER_SIZE), EncodeBitSet() {}

    PairType NEpairTest(uint32_t vertex1, const DecodeInfo &decode_info1, uint32_t vertex2,
    ...
    // use SIMD 
    PairType NonNTestSIMD(uint32_t vertex1, uint32_t vertex2);
     ...
     * */
    void RestructChoose(uint32_t vertex1, std::vector<uint32_t> &neighbor1, uint32_t vertex2,
    ...
    void DynamicChoose(uint32_t vertex, const std::vector<uint32_t> &neighbors, uint32_t &count);

    // compare score and choose the highest, and build the encoding
    void ChooseHighest(const std::vector<uint32_t> &neighbors, uint32_t &max_score, BitSetSIMD &best_bitset,
                       HashCount *&hash_count, uint32_t block_size, uint32_t first_idx, const BlockType &type);

    void SetNonDecodable(const std::vector<uint32_t> &neighbor_encode, const std::vector<uint32_t> &neighbor_left,
                         const BlockType &type, BitSetSIMD &bitset);
    ...
    uint32_t NePairCount(uint32_t vertex_id, const BitSetSIMD &encode);
    ...
    inline bool IsDecodable(uint32_t vertex) { return encode_bitset_[vertex]->IsOne((uint32_t)0); }
    ...
    inline bool IsFull(uint32_t vertex, uint32_t *index_not_full) {
        for (uint32_t index = 1; index <= PER_ENCODE_BIT_SIZE - v_bits_size_ + 1; index += v_bits_size_) {
            if (encode_bitset_[vertex]->BlockGet(index, v_bits_size_) == 0) return false;
        }
        return true;
    }

    inline uint32_t GetBlockSize(uint32_t vertex) { return encode_bitset_[vertex]->BlockGet(3, log_k_); }

    EncodeType GetEncodeType(uint32_t vertex) {
        if (!IsDecodable(vertex)) return EncodeType::NonDecodable;
        if (IsFull(vertex)) return EncodeType::Full;
    ...
    inline uint32_t GetHashBegin(uint32_t vertex) { return GetBlockSize(vertex) * v_bits_size_ + 3 + log_k_; }
