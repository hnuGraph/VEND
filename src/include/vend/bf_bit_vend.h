...
class BFilterBitVend : public Vend {
   public:
    ...
    BFilterBitVend(const std::string &encode_path, std::shared_ptr<DbEngine> &db, int type) : Vend(encode_path, db) {
        if (type == 0)
            encodes_ = std::make_shared<BFilterBitEncode>();
        else if (type == 1)
            encodes_ = std::make_shared<CBFilterEncode>();
    ...
    BFilterBitVend(const std::string &encode_path, std::shared_ptr<DbEngine> &db) : Vend(encode_path, db) {
